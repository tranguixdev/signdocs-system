...
class Document < ApplicationRecord # rubocop:disable Style/Documentation
  ...
  def status
    return FINAL if final.attached?
  ...
  def bucket
    ...
    return client.bucket(ENV['S3_BUCKET_NAME']) if Rails.env.production?

    client.bucket(Rails.application.credentials.aws.dev.bucket)
  end
end
