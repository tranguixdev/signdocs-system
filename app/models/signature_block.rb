...
class SignatureBlock < ApplicationRecord # rubocop:disable Metrics/ClassLength
  ...
  IMGS_PATH = "#{Rails.root}/app/assets/images/" # rubocop:disable Style/MutableConstant
  ...
  def gen_new_pub_key
    ...
    self.pub_key_fingerprint = pub_key.upcase
    # @kms ||= kms
  ...
  def fetch_pub_key(truncated = false)
    key_id = pub_key || new_pub_key
    @kms ||= kms

    key = @kms.get_public_key({ key_id: key_id }).public_key
    return make_checksum(key).upcase if truncated
  ...
  def verify_signature(doc, signature)
    ...
    message = signing_message(doc)
    key_id = signature.key_id || pub_key
    @kms ||= kms
    @kms.verify({
                  key_id: pub_key,
                  message: message,
  ...
  def svg_wrapper
    ...
    fingerprint_text = pub_key_fingerprint
    svg.build do
      svg.path(
